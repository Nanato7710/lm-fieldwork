...
2. [`work/week04/paper_notes.md`](../../work/week04/paper_notes.md)を開き、事前確認欄と論文読解の10項目を確認する。
3. [tasks.md](tasks.md)を開き、Core checklistを上から進める。
4. checklistから[Post-training and evaluation primer](post_training_evaluation_primer.md)、[Chat template primer](chat_template_primer.md)、講師が指定した1本の論文を順に開く。
...
4. [Post-training and evaluation primer](post_training_evaluation_primer.md)を読み、用語の位置付けと評価項目をnotesへ書く。
5. [Chat template primer](chat_template_primer.md)を読み、固定したtoy templateの出力を予想してから観察コードを実行する。
6. 言語処理100本ノックの問94、95を確認演習として読み、chat templateの入力と出力を確認する。
7. 講師指定論文のAbstract、Introduction、figures/tables overviewを2人とも読む。
8. 2人で主要claimと重要なtableを一つずつ決める。
9. Method、Main Results、Ablation、Limitationsの必要箇所へ進む。
10. paper notesを分担した場合も、最後に2人で内容を照合する。
11. 共同発表を準備し、「完了の目安」と照合してfinal meetingへ持っていく。
...
- [Post-training and evaluation primer](post_training_evaluation_primer.md): base/instruct、SFT、preference data、DPO、RLHF/RLと、8つの評価用語を配置する。
- [Chat template primer](chat_template_primer.md): 固定したtoy templateを手作業とローカルコードで追う。
...
1. post-trainingの4項目を使うデータ、weights更新の有無、目的で分類し、8つの評価用語を確認する。
2. 固定したtoy templateを手作業で直列化してから観察コードで確かめ、問94、95でchat templateとmulti-turn promptの役割を確認する。
3. 2人で同じ論文を読み、`work/week04/paper_notes.md`の10項目を埋める。
4. baselineと最重要ablationを1つずつ選び、claimを本当に支えるか議論する。
5. 2人共同で15〜20分、3〜5枚程度の簡易発表を準備する。

## Standard

主要claimとは別のclaimを一つ選び、compute/data budget、contamination、評価者、統計的不確実性まで同じ方法で確認します。
